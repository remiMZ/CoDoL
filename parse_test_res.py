...
def parse_function(*metrics, directory="", args=None, end_signal=None):
    ...
    for output in outputs:
        ...
        with open(osp.join(args._new_path, '{}.txt'.format(args._path[-1])), "a") as f:
            f.write(msg)
            f.write("\r\n")
...
def main(args, end_signal):
    args._path = args.directory.split("/")
    args._new_path = Path(args.directory).parent
    if not os.path.exists(osp.join(args._new_path, '{}.txt'.format(args._path[-1]))):
        os.mknod(osp.join(args._new_path, '{}.txt'.format(args._path[-1])))
