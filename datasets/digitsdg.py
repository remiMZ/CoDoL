...
class DigitsDG(SingleDatasetBase):
    """Digits-DG.
    ...
    def __init__(self, cfg):
        ...
        train_0 = self.read_data(
            self.dataset_dir, cfg.DATASET.SOURCE_DOMAINS_0, "train"
        )
        train_1 = self.read_data(
            self.dataset_dir, cfg.DATASET.SOURCE_DOMAINS_1, "train"
        )
        train_2 = self.read_data(
            self.dataset_dir, cfg.DATASET.SOURCE_DOMAINS_2, "train"
        )
        val = self.read_data(
            self.dataset_dir, cfg.DATASET.SOURCE_DOMAINS, "val"
        )
        test = self.read_data(
            self.dataset_dir, cfg.DATASET.TARGET_DOMAINS, "val"
        )
