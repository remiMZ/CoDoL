...
from dassl.datasets.base_dataset import Datum, DatasetBase

@DATASET_REGISTRY.register()
class VLCSDG(DatasetBase):
    """VLCS.
        ...
        for domain, dname in enumerate(input_domains):
            ...
            for label, folder in enumerate(folders):
                ...
                for impath in impaths:
                    if impath.split("/")[-2] == "0":
                        classname = "bird"
                    elif impath.split("/")[-2] == "1":
                        classname = "car"
                    elif impath.split("/")[-2] == "2":
                        classname = "chair"
                    elif impath.split("/")[-2] == "3":
                        classname = "dog"
                    elif impath.split("/")[-2] == "4":
                        classname = "person"
                    item = Datum(
                        impath=impath, 
                        label=label, 
